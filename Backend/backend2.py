from flask import request, Response, session, jsonify, redirect, url_for, current_app
...
# Function to issue a new JWT token
def issue_new_token(user_id):
    expiration_time = datetime.now(pytz.utc) + timedelta(minutes=30)  # Set expiration time (e.g., 30 minutes from now)
    new_token = jwt.encode({'user_id': user_id, 'exp': expiration_time}, current_app.config['SECRET_KEY'])
    return new_token

# Check token expiry and issue new token if needed
def check_token_expiry_and_refresh(token):
    try:
        decoded_token = jwt.decode(token, current_app.config['SECRET_KEY'], algorithms=['HS256'])
        expiration_time = decoded_token['exp']
        if expiration_time < datetime.now(pytz.utc) + timedelta(minutes=5):  # Refresh token if it's about to expire in less than 5 minutes
            user_id = decoded_token['user_id']
            new_token = issue_new_token(user_id)
            return new_token
    except jwt.ExpiredSignatureError:
        pass  # Token has already expired, no need to refresh
    except jwt.InvalidTokenError:
        pass  # Invalid token, no need to refresh
    return None  # No need to refresh token

# Example usage in a protected resource class
class ProtectedResource(Resource):
    def get(self):
        token = request.args.get('token')
        new_token = check_token_expiry_and_refresh(token)
        if new_token:
...
class UserLogin(Resource):
    ...
    def post(self): 
        ...
        if user and pwd == user.password:
            ...
            token = jwt.encode({'user_id': user.user_id, 'user_role': user_role}, app.config['SECRET_KEY'])
            args['password'] = ""
